...
export default function Courier() {
  ...
  const handleOrderClick = async (orderId: bigint, iterationStatus: number) => {
    try {
      const functionName =
        iterationStatus === 1 ? "takeOrderAsCourier" : 2 ? "markOrderPickedUp" : "markOrderDelivered";
    ...
    if (isOrdersFetched) {
      if (ordersData) {
        ...
        const mutableOrdersData: Order[] = (ordersData as any).map((order: any) => {
          ...
          if (order.storeApproved && !order.courierFound) {
          ...
          } else if (order.courierFound && order.courier === account) {
            ...
            status = "You are your on to way to the Customer";
            iterationStatus = 3;
...
function writeContractAsync(arg0: unknown) {
